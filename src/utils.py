...
def text2json(customize=False):
    ...
    print(os.path)
    if customize:
        text_data_dir = os.path.join('.', 'data', 'text_customize')
        json_data_dir = os.path.join('.', 'data', 'json_customize')
    else:
        text_data_dir = os.path.join('.', 'data', 'text')
        ...
        with io.open(text_file, 'rt', newline='') as file_object:
            for line_count, line in enumerate(file_object, start=1):
                if line_count in [2, 3, 4, 6, 7, 8, 9]:
                    ...
                    json_data['Capacity'] = float(values[1])
                elif line_count == 10:
